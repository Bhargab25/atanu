<div>
    ...
    @if($showCompanySelector)
        <div class="mb-8">
            <x-mary-header title="Company Management" subtitle="Select a company to manage or create a new one" separator />
            
            {{-- Create New Company Button --}}
            <div class="mb-6">
                <x-mary-button label="Create New Company" class="btn-primary" 
                    icon="o-plus" @click="$wire.createNewCompany()" />
            </div>
            ...
            @if($companies->count() > 0)
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($companies as $company)
                        <x-mary-card class="hover:shadow-lg transition-shadow cursor-pointer border-2 {{ $company->is_default ? 'border-green-200 bg-green-50' : 'border-gray-200' }}">
                            <div class="flex justify-between items-start mb-4">
                                <div class="flex-1">
                                    <h3 class="font-semibold text-lg text-gray-900">{{ $company->name }}</h3>
                                    @if($company->legal_name)
                                        <p class="text-sm text-gray-600">{{ $company->legal_name }}</p>
                                    @endif
                                ...
                                @if($company->logo_path)
                                    <img src="{{ Storage::url($company->logo_path) }}" alt="{{ $company->name }} Logo" 
                            ...
                            <div class="flex flex-wrap gap-2">
                                <x-mary-button label="Manage" class="btn-primary btn-sm flex-1" 
                                    @click="$wire.switchToCompany({{ $company->id }})" />
                                
                                @if(!$company->is_default)
                                    <x-mary-button label="Set Default" class="btn-outline btn-sm" 
                                        @click="$wire.setAsDefault({{ $company->id }})" />
                                @endif
                                
                                @if($companies->count() > 1 && !$company->is_default)
                                    <x-mary-button icon="o-trash" class="btn-error btn-sm" 
                                        @click="confirm('Are you sure you want to delete this company?') && $wire.deleteCompany({{ $company->id }})" />
                                @endif
                            </div>
                        </x-mary-card>
                    @endforeach
            ...
            @else
                <div class="text-center py-12">
                    <x-mary-icon name="o-building-office" class="w-16 h-16 text-gray-400 mx-auto mb-4" />
                    ...
                    <x-mary-button label="Create First Company" class="btn-primary" 
                        @click="$wire.createNewCompany()" />
                </div>
            @endif
        </div>

    @elseif($showCreateForm || ($companyProfile && !$companyProfile->exists))
        {{-- Create New Company Form --}}
        <div class="mb-6">
            <x-mary-header title="Create New Company" subtitle="Enter company information to create a new company profile">
                <x-slot:actions>
                    <x-mary-button label="Back to Companies" class="btn-outline" 
                        @click="$wire.showCompanyList()" />
                </x-slot:actions>
            </x-mary-header>
        </div>
    ...
    @else
        ...
        <div class="mb-6">
            <x-mary-header title="Company Profile: {{ $companyProfile->name ?? 'Loading...' }}" 
                subtitle="Manage company information and settings">
                <x-slot:actions>
                    <div class="flex gap-2">
                        @if($companyProfile && !$companyProfile->is_default && $companies->count() > 1)
                            <x-mary-button label="Set as Default" class="btn-outline btn-sm" 
                                @click="$wire.setAsDefault({{ $companyProfile->id }})" />
                        @endif
                        <x-mary-button label="All Companies" class="btn-outline" 
                            @click="$wire.showCompanyList()" />
                        <x-mary-button label="Create New" class="btn-primary" 
                            @click="$wire.createNewCompany()" />
                    </div>
                </x-slot:actions>
            </x-mary-header>
    ...
    {{-- Main Content: Show tabs and forms only when not in company selector mode --}}
    @if(!$showCompanySelector)
        {{-- Tabs Navigation --}}
        <div class="mb-6">
        ...
        {{-- Tab Content (All your existing tab content remains exactly the same) --}}
        @if($activeTab === 'basic')
            <x-mary-card>
